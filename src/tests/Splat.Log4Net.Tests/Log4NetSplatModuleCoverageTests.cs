...
/// <summary>Tests that verify the <see cref="Log4NetSplatModule"/> registers log4net logging.</summary>
public class Log4NetSplatModuleCoverageTests
{
    /// <summary>Verifies that <see cref="Log4NetSplatModule.Configure"/> registers an <see cref="ILogManager"/>.</summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Test]
    public async Task Configure_Registers_ILogManager()
    {
        var resolver = new ModernDependencyResolver();
        var module = new Log4NetSplatModule();
    ...
    public async Task Configure_Registered_Manager_Produces_Logger()
    {
        ...
        var logger = logManager.GetLogger(typeof(Log4NetSplatModuleCoverageTests));
